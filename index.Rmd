...
pwt <- pwt_new %>% select(-pop) %>% arrange(desc(year))
pwt %>% left_join(cpops) %>% summarise_all(function(x)sum(is.na(x)))
pwt %>% left_join(cpops) %>% na.omit -> pwt
nrow(pwt)
```

countrypops has 12470 observations while pwt_sample has 1428 observations. countrypops has 215 different countries while pwt_sample has 21 different countries. countrypops has 195 different countries that pwt_sample doesn't have, while pwt_sample has 1 country that countrypops doesn't have. The two datasets have 20 countries in common. The joined dataset, pwt, has 1160 observations, which is 268 less than that of pwt_sample and 11310 less than that of countrypops. No observations were dropped as a result of the joining. 

####  Wrangling

```{R}
cpops <- countrypops %>% rename(country=country_name) %>% select(-country_code_2, -country_code_3)
pwt_new <- pwt_sample %>% select(-isocode, -X1)
pwt1 <- pwt %>% select(-X1) %>% filter(year>=2000) %>% mutate(gdp_cap = rgdpna/population) %>% arrange(-desc(country), -desc(year)) # gdp_cap to compare the rates at which gdp and pop are changing
...
pwt1 %>% group_by(year) %>% summarise(mean(labsh)) -> labsh_change # change in labor share among 20 "rich" countries from 2000-2017
```
